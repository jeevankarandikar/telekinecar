...
from filtering import filter_epoch
...
def train_classifier(data_file="emg_gestures.pkl"):
    """
    Loads recorded data and trains a gesture classifier using all 6 classes:
    relaxed, clenched_fist, open_palm, upwards_rotation, downwards_rotation, and left_rotation.
    Each epoch is filtered and then flattened into a feature vector.
    Performs 5‑fold cross-validation on the training set and prints the CV scores.
    ...
    # Use all six gestures.
    # (No filtering by allowed gestures.)
...
def real_time_inference(clf, scaler, le, sampling_rate=500, model_input_len=100):
    ...
    try:
        ...
        while True:
            ...
            # Apply filtering to the epoch.
            filtered_emg = filter_epoch(emg_data, sampling_rate)
            
            # Flatten the filtered epoch to create a feature vector.
